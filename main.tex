...
By defining the reference pressure $p^{b}$ as the pressure from the background pressure or from last time step, Equation 17 can be rewritten as:
...
It was found that when the background pressure is used as the reference (frozen) pressure, even a pressure drop as small as 100 Pa can lead to cumulative errors that eventually cause numerical failure in the later stages of the simulation. Therefore, using the pressure from the previous time step as the reference (frozen) pressure yields very stable numerical results.
