...
Although the equation has been simplified in form, the coefficient $C(p)$ still contains highly nonlinear terms related to pressure $p$. To achieve a linearized solution, these coefficients must be "frozen" as constants at a specific reference pressure $\bar{p}$. In this study, we take the pressure from the previous time step, $p^t$, as the reference pressure. Consequently, the final governing equation for gas flow becomes a linear equation with respect to pressure:

\begin{equation}
C_{m i c r o}(\bar{p}) \cdot \frac{\partial p}{\partial t}=\nabla \cdot\left(T_{m i c r o}(\bar{p}) \nabla p\right)
\end{equation}
...
By defining the reference pressure $\bar{p}$ as the pressure from the previous time step, Equation 32 can be rewritten as:

\begin{equation}
V_{i} C_{i, \text{macro}}(\bar{p}) \frac{\partial p_{i}}{\partial t}+\sum_{j=1}^{N_{i}} T_{i j}\left(p_{i}-p_{j}\right)=0
\end{equation}

Equation 16 can then be discretized as follows:

\begin{equation}
V_{i} C_{i, \text{micro}}\left(p_{i}^{t}\right) \frac{p_{i}^{t+\Delta t}-p_{i}^{t}}{\Delta t}+\sum_{j=1}^{N_{i}} T_{i j, \text{micro}}\left(p^{t}\right)\left(p_{j}^{t+\Delta t}-p_{i}^{t+\Delta t}\right)=0
\end{equation}

The transmissibility coefficient between micropores, $T_{i j,micro}\left(p^{t}\right)$, is given by:

\begin{equation}
T_{i j, \text{micro}}\left(p^{t}\right)=\frac{a_{i} a_{j}}{a_{i}+a_{j}}, \text{ with } a_{i}=\frac{A_{i} T_{i, \text{micro}}\left(p_{i}^{t}\right)}{d_{i}} \mathbf{n}_{i} \cdot \mathbf{f}_{i} \text{ and } a_{j}=\frac{A_{j} T_{j, \text{micro}}\left(p_{j}^{t}\right)}{d_{j}} \mathbf{n}_{j} \cdot \mathbf{f}_{j}
\end{equation}

Equation 19 is discretized as:

\begin{equation}
V_{i} C_{i, \text{macro}}\left(p_{i}^{t}\right) \frac{p_{i}^{t+\Delta t}-p_{i}^{t}}{\Delta t}+\sum_{j=1}^{N_{i}} T_{i j, \text{macro}}\left(p^{t}\right)\left(p_{i}^{t+\Delta t}-p_{j}^{t+\Delta t}\right)=0
\end{equation}

The gas transmissibility coefficient  between macropores $T_{i j, \text{macro}}\left(p^{t}\right)$ is defined as:

\begin{equation}
T_{i j, \text{macro}}\left(p^{t}\right)=\frac{\operatorname{fun}_{i}(\mathrm{Kn})+\operatorname{fun}_{j}(\mathrm{Kn})}{2} \frac{\pi R_{i j}^{4}}{8 \mu l_{i j}} * \frac{\rho_{i}^{t}+\rho_{j}^{t}}{2}
\end{equation}
